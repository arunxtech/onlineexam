                ...
                <table class="table table-striped">
                    ...
                    <?php foreach ($Payment as $k => $post):
                        ...
                        <tr>
                            ...
                            <td><?php if ($post['Payment']['status'] == "Approved") {
                                    echo $this->Html->link('<span class="fa fa-arrows-alt"></span>', 'javascript:void(0);', array('onclick' => "show_modal('$viewUrl');", 'escape' => false, 'class' => 'btn btn-info'));
                                } else {
                                    if ($k == 0 && $cartCount > 0) {
                                        echo $this->Html->link(__('Retry'), array('crm' => false, 'controller' => 'Checkouts', 'action' => 'paymentMethod'), array('class' => 'btn btn-info'));
                                    }
                                } ?></td>
                            <td><?php if ($post['Payment']['status'] == "Approved") {
                                    echo $this->Html->link('&nbsp;' . __('Take Exam'), array('controller' => 'Exams', 'action' => 'index'), array('escape' => false, 'class' => 'btn btn-success'));
                                } ?></td>
                        </tr>
                    <?php endforeach;
                    unset($post); ?>

                ...
                <table class="table table-striped">
                    ...
                    <?php foreach ($Result as $post): ?>
                        <tr>
                            ...
                            <td><?php echo CakeNumber::precision($post['Result']['obtained_marks'], 2); ?>
                                /<?php echo CakeNumber::precision($post['Result']['total_marks'], 2); ?></td>
                            <td><?php echo $this->Number->toPercentage($post['Result']['percent']); ?></td>
                            <td>
                                <span class="label label-<?php if ($post['Result']['result'] == "Pass") echo "success"; else echo "danger"; ?>"><?php if ($post['Result']['result'] == "Pass") {
                                        echo __('PASSED');
                                    } else {
                                        echo __('FAILED');
                                    } ?></span></td>
                            <td><?php if ($post['Exam']['declare_result'] == 'Yes') {
                                    echo $this->Html->link('<span class="glyphicon glyphicon-fullscreen"></span>&nbsp;', array('action' => 'view', $post['Result']['id']), array('class' => 'btn btn-default', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => __('View Details'))); ?>
                                    <?php echo $this->Html->link('<span class="glyphicon glyphicon-print"></span>&nbsp;', array('action' => 'printresult', $post['Result']['id']), array('class' => 'btn btn-default', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => __('Print'), 'target' => '_blank'));
                                } ?>

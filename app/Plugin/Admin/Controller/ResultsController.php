...
class ResultsController extends AdminAppController
{
    ...
    public function index($studentId = null)
    {
        try {
            ...
            if (isset($this->request->data['Result']['examWise'])) {
                $examDetails = $this->Result->examWise($this->userGroupWiseId, $name, $studentGroup, $status);
                $this->set('examResult', $examDetails);
                $this->set('isExam', true);
            }
            if (isset($this->request->data['Result']['studentWise'])) {
                $studentDetails = $this->Result->studentWise($this->userGroupWiseId, $name, $studentGroup);
                $this->set('studentDetails', $studentDetails);
                $this->set('isStudent', true);
            }
            if ($studentId != null) {
                $examDetails = $this->Result->studentExamWise($this->userGroupWiseId, $studentId, $studentGroup);
                $this->set('examResult', $examDetails);
    ...
    public function dwstdresult($studentId)
    {
        try {
            $this->layout = 'pdf';
            $examDetails = $this->Result->studentExamWise($this->userGroupWiseId, $studentId, null);

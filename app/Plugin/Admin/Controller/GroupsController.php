        ...
        if ($this->request->is('post')) {
            ...
            try {
                if ($this->Group->save($this->request->data)) {
                    ...
                    $this->Session->setFlash(__('Class Added Successfully'), 'flash', array('alert' => 'success'));
        ...
        if ($this->request->is(array('post', 'put'))) {
            try {
                if ($this->Group->saveAll($this->request->data)) {
                    $this->Session->setFlash(__('Class has been saved'), 'flash', array('alert' => 'success'));
                    return $this->redirect(array('action' => 'index'));
                }
            } catch (Exception $e) {
                $this->Session->setFlash($e->getMessage(), 'flash', array('alert' => 'danger'));
    ...
    public function deleteall()
    {
        try {
            if ($this->request->is('post')) {
                ...
                $this->Session->setFlash(__('Class has been deleted'), 'flash', array('alert' => 'success'));
            }

<div <?php if (!$isError){ ?>class="container"<?php } ?>>
    <div class="panel panel-custom mrg">
        <div class="panel-heading"><?php echo __('Edit Users'); ?><?php if (!$isError) { ?>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><?php } ?>
        </div>
        <div class="panel-body"><?php echo $this->Session->flash(); ?>
            <?php echo $this->Form->create('User', array('class' => 'form-horizontal')); ?>
            <?php foreach ($User as $k => $post): $id = $post['User']['id'];
                $form_no = $k + 1; ?>
                <div class="panel panel-default">
                    <div class="panel-heading"><strong>
                            <small class="text-danger"><?php echo __('User Registration Form'); ?><?php echo $form_no ?></small>
                        </strong></div>
                    <div class="panel-body">
                        <?php if ($id != 1) { ?>
                            <div class="form-group">
                                <label for="group_name"
                                       class="col-sm-3 control-label"><?php echo __('User Level'); ?></label>
                                <div class="col-sm-9">
                                    <?php echo $this->Form->select("$k.User.ugroup_id", $ugroup, array('empty' => null, 'label' => false, 'class' => 'form-control', 'div' => false)); ?>
                                </div>
                            </div>
                            <div class="form-group">
                            <label for="group_name" class="col-sm-3 control-label"><?php echo __('Class'); ?></label>
                            <div class="col-sm-9">
                                <?php $gp2 = array();
                                ...
                                <?php endforeach;
                                unset($groupName); ?>
                                <?php echo $this->Form->select("$k.UserGroup.group_name", $group_id, array('value' => $gp2, 'multiple' => true, 'label' => false, 'class' => 'form-control multiselectgrp', 'div' => false));
                                $gp2 = array(); ?>
                            </div>
                            </div><?php } ?>
                        <div class="form-group">
                        ...
                        <?php if ($id != 1) { ?>
                            <div class="form-group">
                                <label for="group_name"
                                       class="col-sm-3 control-label"><?php echo __('Status'); ?></label>
                                <div class="col-sm-9">
                ...
                <div class="col-sm-offset-3 col-sm-7">
                    ...
                    <?php if (!$isError) { ?>
                        <button type="button" class="btn btn-danger" data-dismiss="modal"><span
                                class="glyphicon glyphicon-remove"></span>&nbsp;<?php echo __('Cancel'); ?>
                        </button><?php } else {

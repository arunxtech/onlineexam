...
class Result extends AppModel
{
    ...
    public function userMarksheet($id)
    {
        ...
        $totalUnattemptedQuestionMarks = 0;
    ...
    {
        $ExamStat = ClassRegistry::init('ExamStat');
        $subjectNameArr = $ExamStat->find('all', array(
            ...
            'conditions' => array('ExamStat.exam_result_id' => $id, 'ExamStat.student_id' => $studentId, 'closed' => 1),
            'group' => array('Question.subject_id'),
            'order' => array('ExamStat.ques_no asc')));
        foreach ($subjectNameArr as $value) {
    ...
    public function userExamQuestion($id, $studentId, $bookmark = 'N')
    {
        $ExamStat = ClassRegistry::init('ExamStat');
        ...
        $Question->bindModel(array('belongsTo' => array('Subject', 'Qtype', 'Diff', 'Passage')));
        $ExamStat->bindModel(array('belongsTo' => array('Question', 'Exam')));
        $cond = array();
        if ($bookmark == "Y") {
            $cond = array('ExamStat.bookmark' => 'Y');
        }
        $userExamQuestion = $ExamStat->find('all', array('recursive' => 2,
            'conditions' => array('ExamStat.exam_result_id' => $id, 'ExamStat.student_id' => $studentId, $cond),
            'order' => array('ExamStat.ques_no asc')));
        $ExamStat->unbindModel(array());
        $ExamStat->unbindModel(array());

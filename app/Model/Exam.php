...
class Exam extends AppModel
{
    ...
    public function getPurchasedExam($type, $studentId, $currentDateTime, $limit = NULL, $offset = 0)
    {
        ...
        $this->virtualFields = array('attempt' => 'SELECT COUNT(ExamResult.id) FROM `exam_results` AS `ExamResult` WHERE `ExamResult`.`exam_id`=`Exam.id` AND `ExamResult`.`student_id`=' . $studentId,
            ...
            'fexpiry_date' => 'PackagesPayment.expiry_date',
        );

        $examList = $this->find('all', array(
            'fields' => array('Exam.attempt', 'Exam.attempt_order', 'Exam.fexpiry_date', 'Package.id', 'Exam.id', 'Exam.type', 'Exam.name', 'Exam.start_date', 'Exam.end_date', 'Exam.paid_exam', 'Exam.amount', 'Exam.attempt_count', 'Exam.expiry', 'Package.name'),
            'joins' => array(
                ...
                array('table' => 'payments', 'alias' => 'Payment', 'type' => 'INNER', 'conditions' => array('Payment.student_id' => $studentId)),
                ...
                array('table' => 'packages', 'alias' => 'Package', 'type' => 'INNER', 'conditions' => array('Package.id=ExamsPackage.package_id')),
            ),
            'conditions' => array($start_date, $end_date, $expiredDate, 'Exam.status' => 'Active', 'Exam.user_id' => 0, 'Payment.status' => 'Approved', 'Payment.student_id' => $studentId, 'Package.package_type <>' => 'F', 'ExamGroup.group_id' => $mainStudentGroup),
    ...
    public function getFreeExam($studentId, $currentDateTime, $limit = NULL, $offset = 0)
    {
        ...
        $examList = $this->find('all', array(
            ...
            'joins' => array(
                array('table' => 'exam_preps', 'alias' => 'ExamPrep', 'type' => 'LEFT', 'conditions' => array('Exam.id=ExamPrep.exam_id')),
                array('table' => 'exam_groups', 'alias' => 'ExamGroup', 'type' => 'Inner', 'conditions' => array('Exam.id=ExamGroup.exam_id')),
                array('table' => 'payments', 'alias' => 'Payment', 'type' => 'LEFT', 'conditions' => array('Payment.student_id' => $studentId)),
                array('table' => 'packages_payments', 'alias' => 'PackagesPayment', 'type' => 'LEFT', 'conditions' => array('Payment.id=PackagesPayment.payment_id')),
                array('table' => 'exams_packages', 'alias' => 'ExamsPackage', 'type' => 'LEFT', 'conditions' => array('Exam.id=ExamsPackage.exam_id', 'PackagesPayment.package_id=ExamsPackage.package_id')),
                array('table' => 'packages', 'alias' => 'Package', 'type' => 'LEFT', 'conditions' => array('Package.id=ExamsPackage.package_id')),
            ),
            'conditions' => array($start_date, $end_date, $expiredDate, 'Exam.status' => 'Active', 'Exam.user_id' => 0, 'Package.package_type' => 'F', 'ExamGroup.group_id' => $mainStudentGroup),
            'order' => array('Exam.start_date' => 'asc'),
            'group' => array("Exam.id"),
            'limit' => $limit,
            'offset' => $offset));
    ...
    public function insertFreeExam($studentId, $currentDateTime, $limit = NULL, $offset = 0)
    {
        ...
        $examList = $this->find('all', array(
            'fields' => array('Exam.attempt', 'Exam.attempt_order', 'Exam.fexpiry_date', 'Package.id', 'Exam.id', 'Exam.type', 'Exam.name', 'Exam.start_date', 'Exam.end_date', 'Exam.paid_exam', 'Exam.amount', 'Exam.attempt_count', 'Exam.expiry', 'Package.name', 'PackagesPayment.expiry_date'),
            'joins' => array(
                array('table' => 'exam_preps', 'alias' => 'ExamPrep', 'type' => 'LEFT', 'conditions' => array('Exam.id=ExamPrep.exam_id')),
                array('table' => 'exam_groups', 'alias' => 'ExamGroup', 'type' => 'Inner', 'conditions' => array('Exam.id=ExamGroup.exam_id')),
                array('table' => 'payments', 'alias' => 'Payment', 'type' => 'LEFT', 'conditions' => array('Payment.student_id' => $studentId)),
                array('table' => 'packages_payments', 'alias' => 'PackagesPayment', 'type' => 'LEFT', 'conditions' => array('Payment.id=PackagesPayment.payment_id')),
                array('table' => 'exams_packages', 'alias' => 'ExamsPackage', 'type' => 'LEFT', 'conditions' => array('Exam.id=ExamsPackage.exam_id')),
                array('table' => 'packages', 'alias' => 'Package', 'type' => 'LEFT', 'conditions' => array('Package.id=ExamsPackage.package_id')),
            ),
            'conditions' => array($start_date, $end_date, $expiredDate, 'Exam.status' => 'Active', 'Exam.user_id' => 0, 'Package.package_type' => 'F'),
            'order' => array('Exam.start_date' => 'asc'),
            'group' => array("Exam.id"),
            'limit' => $limit,
            'offset' => $offset));
    ...
    {
        $ExamStat = ClassRegistry::init('ExamStat');
        $subjectName = $ExamStat->find('all', array('fields' => array('DISTINCT(Subject.subject_name)', 'Subject.id'),

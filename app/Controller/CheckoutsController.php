...
class CheckoutsController extends AppController
{
    ...
    public function beforeFilter()
    {
        ...
        $this->userId = $this->userValue['Student']['id'];
        $this->PPL = false;
        $this->CAE = false;
        $this->PME = false;
    ...
    public function payumoney()
    {
        ...
        if ($recordArr) {
            ...
            $curl = $this->siteDomain . '/Checkouts/index/';
            $firstname = $this->userValue['Student']['name'];
            $email = $this->userValue['Student']['email'];
            $address1 = $this->userValue['Student']['address'];
            ...
            $this->paymentCreate($txnid, $amount, $description, $this->paymentNamePME, 'PME', 'web');
        }
    }
    ...
    public function payumoneypostpayment()
    {
        if (!$_POST) {
            ...
            $this->redirect(array('crm' => true, 'controller' => 'Payments', 'action' => 'paymentMethod'));
        }
        $status = $_POST['status'];
        ...
        if ($hash != $posted_hash) {
            $this->Session->setFlash(__("Security Error. Illegal access detected"), 'flash', array('alert' => 'danger'));
            $this->redirect(array('controller' => 'Payments', 'action' => 'paymentMethod'));
        }
        if ($status === "success") {
            $mobileStatus = "Success";
            $transactionId = $txnid;
            ...
            if ($paymentArr) {
                $this->Payment->save(array('id' => $paymentArr['Payment']['id'], 'status' => 'Cancelled'));
            }
        ...
        if ($this->Session->check('couponArr')) {
            $this->loadModel('CouponsStudent');
            $this->CouponsStudent->updateAll(array('status' => 1), array('session_id' => session_id(), 'status' => 0));
            $this->Session->write('couponArr', null);
        }
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
            $this->redirect(array('rest' => true, 'controller' => 'Checkouts', 'action' => 'success3050', $mobileStatus));
    ...
    public function ccavenue()
    {
        ...
        if ($recordArr) {
            ...
            if (isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
                $this->paymentCreate($orderId, $amount, $description, $this->paymentNameCAE, 'CAE', 'mobile');
            } else {
    ...
    private function orderComplete($token, $transactionId, $correlationId = null, $timestamp = null)
    {
        ...
        if ($paymentArr) {
            ...
            $this->orderEmail($packageDetail, $transactionId, $paymentArr['Payment']['amount']);
            $this->Cart->savePackage(null);
            if ($this->Session->check('couponArr')) {
                $this->loadModel('CouponsStudent');
                $this->CouponsStudent->updateAll(array('status' => 1), array('session_id' => session_id(), 'status' => 0));
    ...
    private function orderEmail($packageDetail, $transactionId, $totalAmount)
    {
        ...
        try {
            ...
            $couponTotalAmount = $this->couponDiscountAmount();
            $couponDiscount = $couponTotalAmount;
            $netAmount = $totalAmount;
            $totalAmount = $totalAmount + $couponTotalAmount;
            /* Send Email */
